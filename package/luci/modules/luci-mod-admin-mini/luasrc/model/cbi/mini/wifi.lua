...
function mode.write(self, section, value)
	if value == "sta" then
		local oldif = m.uci:get("network", "wifi", "ifname")
		if oldif and oldif ~= " " then
			m.uci:set("network", "wifi", "_ifname", oldif)
		end
		m.uci:set("network", "wifi", "ifname", "wlan0")

		self.map:set(section, "network", "wifi")
	else
		if m.uci:get("network", "wifi", "_ifname") then
			m.uci:set("network", "wifi", "ifname", m.uci:get("network", "wifi", "_ifname"))
		end
		self.map:set(section, "network", "wifi")
	end
