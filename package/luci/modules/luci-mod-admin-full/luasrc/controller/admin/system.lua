...
function action_flashops()
	...
	elseif luci.http.formvalue("image") or luci.http.formvalue("step") then
		...
		elseif step == 2 then
			...
			luci.template.render("admin_system/applyreboot", {
				...
				addr  = (#keep > 0) and "192.168.3.1" or nil
			})
			fork_exec("killall uhttpd; sleep 1; mtd -r write /tmp/firmware.img firmware")
		end
	elseif reset_avail and luci.http.formvalue("reset") then
		...
		fork_exec("killall uhttpd; sleep 1; mtd -r erase rootfs_data")
	else
		--
		-- Overview
		--
